export const defaultConfig = {
  styles: {
    use_bonbon_colors: true,
    primary_accent_color: '#009ac7',
    card_background_color_light: '#fff',
    card_background_color_dark: '#222',
    card_text_color_light: '#111',
    card_text_color_dark: '#eee',
    background_image_light: null,
    background_image_dark: null,
  },
  actions: {
    default: {
      icon: 'none',
      button: 'auto',
    },
  },
  views: {
    bonbon_home: {
      max_columns: 1,
      sections: {
        bonbon_weather: {
          name: 'auto',
          icon: 'auto',
          order: 1,
          column: 'auto',
          hide_separator: false,
          disabled: false,
          show_if_empty: true,
          separator_buttons: 'weather.*',
          show_forecast: false,
        },
        bonbon_persons: {
          name: 'Persons',
          icon: 'mdi:account-group',
          order: 2,
          column: 'auto',
          hide_separator: true,
          min_columns: 1,
          max_columns: 2,
          disabled: false,
          cards: 'person.*',
        },
        bonbon_favorites: {
          name: 'Favorites',
          icon: 'mdi:star',
          order: 3,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 2,
          disabled: false,
          cards: '*[label=favorite]',
        },
        bonbon_areas: {
          name: 'Areas',
          icon: 'mdi:sofa',
          order: 4,
          column: 'auto',
          hide_separator: false,
          min_columns: 2,
          max_columns: 2,
          disabled: false,
          bonbon_colors: true,
          separator_buttons: 'light.*:not([label=nightlight])',
          separator_combine_lights: 'when-on',
          sub_buttons: 'light.*:not([label=nightlight])',
          sub_combine_lights: 'when-on',
          inline_buttons: [
            'area.temperature_entity_id',
            'area.humidity_entity_id',
            'sensor.*[device_class=carbon_dioxide][unit_of_measurement=ppm]',
          ],
        },
        bonbon_robots: {
          name: 'Robots',
          icon: 'mdi:robot',
          order: 5,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 2,
          disabled: false,
          cards: ['vacuum.*', 'lawn_mower.*'],
        },
        bonbon_security: {
          name: 'Security',
          icon: 'mdi:lock',
          order: 6,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 1,
          disabled: false,
          cards: ['lock.*'],
        },
      },
    },
    bonbon_area: {
      max_columns: 1,
      sections: {
        bonbon_environment: {
          name: 'Environment',
          icon: 'mdi:thermometer-water',
          order: 1,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 3,
          show_graphs: true,
          cards: [
            'area.temperature_entity_id',
            'area.humidity_entity_id',
            'sensor.*[device_class=carbon_dioxide][unit_of_measurement=ppm]',
          ],
          disabled: false,
        },
        bonbon_scenes: {
          name: 'Scenes & Scripts',
          icon: 'mdi:palette',
          order: 2,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 2,
          disabled: false,
          cards: ['scene.*', 'script.*'],
        },
        bonbon_lights: {
          name: 'Lights',
          icon: 'mdi:lightbulb-group',
          order: 3,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 2,
          disabled: false,
          cards: 'light.*',
          separator_buttons: 'light.*:not([label=nightlight])',
          separator_combine_lights: 'always',
        },
        bonbon_switches: {
          name: 'Switches & Buttons',
          icon: 'mdi:toggle-switch',
          order: 4,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 2,
          disabled: false,
          cards: ['switch.*', 'button.*', 'input_button.*', 'input_boolean.*', 'select.*'],
        },
        bonbon_media: {
          name: 'Media Players',
          icon: 'mdi:disc-player',
          order: 5,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 1,
          disabled: false,
          cards: 'media_player.*',
        },
        bonbon_climate: {
          name: 'Climate & Fans',
          icon: 'mdi:radiator',
          order: 6,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 1,
          disabled: false,
          cards: ['climate.*', 'fan.*', 'humidifier.*'],
        },
        bonbon_covers: {
          name: 'Shutters & Shades',
          icon: 'mdi:roller-shade',
          order: 7,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 1,
          disabled: false,
          cards: 'cover.*',
        },
        bonbon_openings: {
          name: 'Doors & Windows',
          icon: 'mdi:window-closed-variant',
          order: 8,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 2,
          disabled: false,
          cards: 'binary_sensor.*[device_class=door|garage_door|window|opening]',
        },
        bonbon_valves: {
          name: 'Water Systems',
          icon: 'mdi:pipe-valve',
          order: 9,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 2,
          disabled: false,
          cards: ['valve.*', 'water_heater.*'],
        },
        bonbon_miscellaneous: {
          name: 'Miscellaneous',
          icon: 'mdi:dots-horizontal-circle-outline',
          order: Infinity,
          column: 'auto',
          hide_separator: false,
          min_columns: 1,
          max_columns: 2,
          disabled: false,
          cards: '*',
        },
      },
    },
  },
};
